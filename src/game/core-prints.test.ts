...
describe('sector-gated Core prints', () => {
  ...
  it('tiers fragment needs by print unlock sector', () => {
    expect(printFragmentNeeds(2)).toEqual({ casing: 2, core: 1, lens: 1 })
    expect(printFragmentNeeds(6)).toEqual({ casing: 3, core: 2, lens: 1 })
    expect(printFragmentNeeds(12)).toEqual({ casing: 4, core: 3, lens: 2 })
    expect(printFragmentNeeds(18)).toEqual({ casing: 5, core: 4, lens: 3 })
  ...
  it('derives Heavy Lance farm source from the armored drop table', () => {
    expect(formatPrintSourceLine('heavy-lance')).toMatch(/Armored · Sector 2\+/)
  })

  it('lets Sector 4 Divine drop Charge Prism instead of sitting empty', () => {
    expect(formatPrintSourceLine('charge-prism')).toMatch(/Divine · Sector 4\+/)
    expect(pickWeightedDropEntry('divine', 4, () => 0)?.moduleId).toBe('charge-prism')
  })
  ...
  it('biases eligible rolls toward the tracked print', () => {
    ...
    let other = 0
    for (let i = 0; i < 400; i++) {
      const drop = pickWeightedDropEntry('swarm', 6, rng, { trackedModuleId: 'flak-array' })
  ...
  it('biases tracked part types toward missing pieces', () => {
    ...
    for (let i = 0; i < 300; i++) {
      const drop = pickWeightedDropEntry('armored', 2, rng, {
  ...
  it('Hold raises fragment chance only for a tracked eligible Core', () => {
    ...
    advance.meta.highestSectorEver = 2
    advance.combat.highestSector = 2
    advance.combat.sector = 2
